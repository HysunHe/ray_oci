...
def query_instances_by_tags(tag_filters, region):
...
def terminate_instances_by_tags(tag_filters, region) -> int:
    logger.debug(f"Terminate instance by tags: {tag_filters}")
    insts = query_instances_by_tags(tag_filters, region)
    fail_count = 0
    for inst in insts:
...
@utils.debug_enabled(logger=logger)
def subscribe_image(compartment_id, listing_id, resource_version,
                    region):
...
@utils.debug_enabled(logger=logger)
def find_compartment(region) -> str:
...
@utils.debug_enabled(logger=logger)
def find_create_vcn_subnet(region) -> Optional[str]:
    ...
    skypilot_compartment = find_compartment(region)
    ...
    if len(vcns) > 0:
        ...
        return create_vcn_subnet(net_client, skypilot_compartment)


@utils.debug_enabled(logger=logger)
def create_vcn_subnet(net_client,
    ...
    try:
        ...
        delete_vcn(net_client, skypilot_vcn, subnet, ig, sg)
        subnet = None
...
@utils.debug_enabled(logger=logger)
def delete_vcn(net_client, skypilot_vcn, skypilot_subnet,
